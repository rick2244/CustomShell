...
execute_pipeline(struct command *cmd)
{
  ...
  if (cmd->stdout_pipe){
    ...
    if(cmd->stdout_file != NULL){
      ...
      int file_fd = open(cmd->stdout_file, open_flags);
      if (file_fd == -1) {
    ...
    exec(cmd->tokens[0], cmd->tokens); //execute command if there is no need to pipe
    close(fd[0]);
    close(fd[1]);
    return;
  }
  ...
  //Steps for O_APPEND
  //1. we add a new flag in fcntl.h
  //2. modify sysfile.c, modify open
  //3. you want to change the pointer to point and the end if the append flag is set
}
